...
2024-01-02,Press_01,175,96.8"""
    ...
    def test_streamlit_file_upload_mock(self, mock_uploader):
        ...
        mock_file.read.return_value = b"Datum,Maschine,Produktion\n2024-01-01,Laser_01,245"
...
@pytest.mark.skipif(not STREAMLIT_AVAILABLE or not PLOTLY_AVAILABLE, reason="Streamlit oder Plotly nicht verfügbar")
class TestStreamlitCharts:
    ...
    def test_bar_chart_creation(self, sample_time_series_data):
        ...
        agg_data = sample_time_series_data.groupby('Maschine')['Produktion'].sum().reset_index()
...
class TestStreamlitDashboard:
    ...
    def test_alert_generation(self, dashboard_data):
        ...
        def generate_alerts(data):
            ...
            # Wartungs-Alert
            maintenance_machines = [m for m in data['machines'] if m['status'] == 'Maintenance']
    ...
    def test_metric_display(self, mock_metric, dashboard_data):
        ...
        mock_metric("Durchschnittliche Qualität", f"{kpis['avg_quality']:.1f}%", "↑ 0.2%")
