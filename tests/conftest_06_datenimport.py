    ...
    return pd.DataFrame(
        {
            ...
            "Status": np.random.choice(
                ["OK", "WARNING", "ERROR"], 100, p=[0.8, 0.15, 0.05]
            ),
        }
    )
...
def sample_csv_files(test_data_directory, sample_csv_data):
    ...
    sample_csv_data.to_csv(
        german_file, index=False, sep=";", decimal=",", encoding="utf-8"
    )
    ...
    maintenance_data = pd.DataFrame(
        {
            ...
            "Wartungstyp": np.random.choice(
                ["Präventiv", "Korrektiv", "Inspektion"], 50
            ),
            "Dauer_Stunden": np.round(np.random.exponential(2.5, 50), 1),
            "Kosten_EUR": np.round(np.random.gamma(2, 300, 50), 2),
            "Techniker": np.random.choice(
                ["Schmidt", "Müller", "Weber", "Fischer"], 50
            ),
        }
    )
        ...
        "sensors": [
            {
                ...
                "readings": [
                    ...
                    {
                        "timestamp": "2024-01-15T08:34:00Z",
                        "value": 23.7,
                        "status": "warning",
                        "quality": 85,
                    },
    ...
    return pd.DataFrame(
        {
            ...
            "Kategorie": np.random.choice(
                ["Production", "Quality", "Maintenance"], size, p=[0.7, 0.2, 0.1]
            ),
        }
    )
...
def expected_kpis():
    ...
    return {
        "total_production_range": (
            250000,
            350000,
        ),  # Erwarteter Bereich für Gesamtproduktion
...
def assert_dataframe_quality(df, min_rows=1, required_columns=None, allow_na=True):
    ...
    assert (
        len(df) >= min_rows
    ), f"DataFrame hat nur {len(df)} Zeilen, erwartet mindestens {min_rows}"
...
def validate_file_structure(
    file_path, expected_extension=None, min_size=0, max_size=None
):
    """
    ...
    if expected_extension:
        assert (
            file_path.suffix == expected_extension
        ), f"Falsche Erweiterung: {file_path.suffix}, erwartet {expected_extension}"

    file_size = file_path.stat().st_size
    assert (
        file_size >= min_size
    ), f"Datei zu klein: {file_size} Bytes, erwartet mindestens {min_size}"

    if max_size:
        assert (
            file_size <= max_size
        ), f"Datei zu groß: {file_size} Bytes, maximum {max_size}"
