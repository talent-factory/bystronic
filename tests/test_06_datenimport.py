...
try:
    ...
    pytest.skip(
        f"Datenimport Beispiele können nicht importiert werden: {e}", allow_module_level=True
    )
...
class TestCSVImportGrundlagen:
    ...
    def test_csv_fehlende_werte(self):
        ...
        df_filled = df.fillna({'Produktion': df['Produktion'].mean(), 
            ...
            "sensors": [
                {
                    ...
                    "type": "temperature",
                    "readings": [
                        {"timestamp": "2024-01-15T08:30:00Z", "value": 22.5, "status": "ok"},
                        {"timestamp": "2024-01-15T08:31:00Z", "value": 22.8, "status": "ok"}
                    ]
                },
                {
                    "id": "VIBR_001", 
                    ...
                    "readings": [
                        ...
                        {"timestamp": "2024-01-15T08:31:00Z", "value": 2.8, "status": "error"}
                    ]
                }
            ]
        }
...
class TestDatenbereinigung:
    ...
    def test_data_cleaning(self):
        ...
        df_clean.loc[(df_clean['Temperatur'] < -50) | (df_clean['Temperatur'] > 100), 'Temperatur'] = np.nan
...
class TestExportFunktionen:
    ...
    def test_excel_export_multi_sheet(self):
        ...
        with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
            ...
            agg_data = df.groupby('Kategorie').agg({
                'Wert1': 'mean',
                'Wert2': 'sum'
            }).reset_index()
...
class TestIntegration:
    ...
    def test_complete_pipeline(self):
        ...
        # 4. Aggregationen
        daily_stats = df_clean.groupby('Maschine').agg({
            'Produktion': ['mean', 'sum', 'count'],
            'Temperatur': ['mean', 'std']
        }).round(2)
