...
class BystronicCSVParser:
    ...
    def detect_delimiter(self, file_path: str, encoding: str, sample_lines: int = 10) -> str:
        """
        ...
        for delimiter in self.common_delimiters:
            ...
            if scores:
                ...
                consistency_score = avg_count / (std_count + 1)  # +1 zur Vermeidung Division durch 0
                delimiter_scores[delimiter] = consistency_score
        
        if delimiter_scores:
            best_delimiter = max(delimiter_scores.items(), key=lambda x: x[1])[0]
            print(f"✅ Trennzeichen erkannt: {'Tab' if best_delimiter == '\\t' else repr(best_delimiter)}")
            return best_delimiter
        
        print("⚠️ Fallback auf Tab-Trennzeichen")
        return '\t'
    
    def analyze_structure(self, file_path: str, encoding: str = None, delimiter: str = None) -> Dict:
        """
        ...
        for i, line in enumerate(lines):
            ...
            if any(keyword in line_content for keyword in ['File', 'Starttime', 'Endtime', 'Data-Type']):
        ...
        for i, line in enumerate(lines):
            line_content = line.strip()
            if line_content and not any(keyword in line_content for keyword in 
                                      ['Name', 'Data-Type', 'SampleTime', 'SymbolComment', 'File', 'Starttime']):
    ...
    def parse_complex_csv(self, 
        ...
        if header_line and data_start_line and columns:
            try:
                with open(file_path, 'r', encoding=structure_info['encoding'], errors='ignore') as f:
                    lines = f.readlines()
    ...
    def export_parsing_report(self, output_path: str) -> None:
        ...
        report = {
            'summary': {
                ...
                'successful_parses': sum(1 for p in self.parsing_history if p['success']),
                'total_rows_processed': sum(p['rows_parsed'] for p in self.parsing_history),
...
def main():
    ...
    if Path(csv_file).exists():
        ...
        result = parser.parse_complex_csv(get_data_path("examples", "sample_complex.csv"))
