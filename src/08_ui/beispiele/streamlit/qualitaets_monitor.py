...
# Seitenkonfiguration
st.set_page_config(
    page_title="Bystronic Qualitäts-Monitor",
    page_icon="🔍",
    layout="wide"
)
...
def main():
    ...
    def generate_quality_data():
        np.random.seed(42)
        timestamps = pd.date_range(start=datetime.now()-timedelta(hours=24), end=datetime.now(), freq='5min')
    ...
    st.sidebar.selectbox(
        "Zeitraum:",
        ["Letzte 4 Stunden", "Letzten 8 Stunden", "Letzten 24 Stunden"]
    )
    ...
    # Hauptinhalt
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Übersicht", "📈 SPC Charts", "🔍 Detailanalyse", "📋 Prüfprotokoll"])
...
def show_quality_overview(df):
    ...
    hourly_quality = df.groupby(df['timestamp'].dt.floor('H')).agg({
    ...
    fig_trend = px.line(hourly_quality, x='timestamp', y='pass_rate',
                       title='Gutteile-Rate über Zeit',
                       labels={'pass_rate': 'Gutteile-Rate [%]'})
    fig_trend.add_hline(y=95, line_dash="dash", line_color="red", annotation_text="Zielwert")
    ...
    with col1:
        fig_dist = px.histogram(df, x='dimension_x', title='Verteilung Maß X',
                               labels={'dimension_x': 'Maß X [mm]'})
        fig_dist.add_vline(x=100.0, line_dash="dash", line_color="green", annotation_text="Sollmaß")
        fig_dist.add_vline(x=99.7, line_dash="dot", line_color="red", annotation_text="Untergrenze")
        fig_dist.add_vline(x=100.3, line_dash="dot", line_color="red", annotation_text="Obergrenze")
        st.plotly_chart(fig_dist, use_container_width=True)

    with col2:
        fig_scatter = px.scatter(df, x='dimension_x', y='dimension_y', color='pass_fail',
...
def show_spc_charts(df):
    ...
    # Parameter auswählen
    parameter = st.selectbox("Parameter für SPC:",
    ...
    fig_xbar = go.Figure()
    fig_xbar.add_trace(go.Scatter(x=df['timestamp'], y=df[parameter], mode='lines+markers', name='Messwerte'))
    ...
    fig_xbar.update_layout(title=f"X-Chart für {parameter}", xaxis_title="Zeit", yaxis_title="Wert")
    ...
    fig_mr = go.Figure()
    fig_mr.add_trace(go.Scatter(x=df['timestamp'][1:], y=moving_range, mode='lines+markers', name='Moving Range'))
    fig_mr.add_hline(y=mr_mean, line_color="green", annotation_text="MR Mittelwert")
    fig_mr.add_hline(y=mr_ucl, line_color="red", line_dash="dash", annotation_text="UCL")
    fig_mr.update_layout(title=f"Moving Range Chart für {parameter}", xaxis_title="Zeit", yaxis_title="Moving Range")
    ...
    with col2:
        ...
        fig_hist.add_vline(x=usl, line_color="red", line_dash="dash", annotation_text="OSG")
        fig_hist.add_vline(x=lsl, line_color="red", line_dash="dash", annotation_text="USG")
...
def show_detailed_analysis(df):
    ...
    fig_corr = px.imshow(correlation_matrix, title="Korrelationsmatrix Qualitätsparameter",
    ...
    if not failed_parts.empty:
        error_causes = {
            'Maßabweichung X': np.sum((failed_parts['dimension_x'] < 99.7) | (failed_parts['dimension_x'] > 100.3)),
            'Maßabweichung Y': np.sum((failed_parts['dimension_y'] < 49.85) | (failed_parts['dimension_y'] > 50.15)),
            'Oberflächenfehler': np.sum(failed_parts['surface_roughness'] > 2.5),
            'Härteproblem': np.sum((failed_parts['hardness'] < 50) | (failed_parts['hardness'] > 60)),
            'Sonstige': len(failed_parts) // 4
        }

        pareto_df = pd.DataFrame(list(error_causes.items()), columns=['Fehlerursache', 'Anzahl'])
        pareto_df = pareto_df.sort_values('Anzahl', ascending=False)
        pareto_df['Kumulativ %'] = pareto_df['Anzahl'].cumsum() / pareto_df['Anzahl'].sum() * 100

        fig_pareto = make_subplots(specs=[[{"secondary_y": True}]])
        fig_pareto.add_trace(go.Bar(x=pareto_df['Fehlerursache'], y=pareto_df['Anzahl'], name='Anzahl Fehler'))
        fig_pareto.add_trace(go.Scatter(x=pareto_df['Fehlerursache'], y=pareto_df['Kumulativ %'],
                                       mode='lines+markers', name='Kumulativ %'), secondary_y=True)
    ...
    machine_quality = df.groupby('machine').agg({
        'pass_fail': lambda x: (x == 'Pass').mean() * 100,
    ...
    machine_quality.columns = ['_'.join(col).strip() for col in machine_quality.columns.values]
    machine_quality = machine_quality.rename(columns={'pass_fail_<lambda>': 'Gutteile_Rate'})

    st.dataframe(machine_quality, use_container_width=True)

    # Box-Plots für Maschinen-Vergleich
    fig_box = px.box(df, x='machine', y='dimension_x', title="Maßhaltigkeit nach Maschine")
...
def show_inspection_log(df):
    ...
    with col2:
        selected_inspector = st.selectbox("Prüfer:", ['Alle'] + list(df['inspector'].unique()))
    ...
    with col1:
        if st.button("📋 Prüfprotokoll als CSV"):
            ...
            st.download_button("Download CSV", csv_data, "pruefprotokoll.csv", "text/csv")

    with col2:
        if st.button("📊 Qualitätsbericht generieren"):
            st.success("Qualitätsbericht wurde generiert!")
            st.info("Der Bericht enthält alle relevanten Qualitätsmetriken und SPC-Charts.")
