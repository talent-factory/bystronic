...
class ProductionData:
        ...
        for i in range(100):
            ...
            for machine in machines:
                ...
                data.append({
                    ...
                    'operator': np.random.choice(['Schmidt', 'Müller', 'Weber', 'Meyer']),
    ...
    def generate_production_orders():
        ...
        order_types = [
            'Flansch 150mm', 'Gehäuse Typ A', 'Blech 5mm Steel',
            'Halterung verzinkt', 'Träger L-Profil', 'Platte 10mm Alu'
        ]
        ...
        for i in range(15):
            ...
            orders.append({
                ...
                'machine_assigned': np.random.choice(['Laser 1', 'Laser 2', 'Stanze 1', 'Biegemaschine']),
                'operator': np.random.choice(['Schmidt', 'Müller', 'Weber', 'Meyer']),
                'priority': np.random.choice(priorities, p=[0.6, 0.3, 0.1]),
                'status': np.random.choice(statuses, p=[0.2, 0.3, 0.1, 0.3, 0.1]),
                'start_time': start_time,
                'estimated_end': start_time + timedelta(hours=np.random.randint(2, 12)),
...
def main():
    ...
    # Titel
    st.markdown('<div class="main-title">🏭 Bystronic Produktions-Dashboard</div>',
                unsafe_allow_html=True)
    ...
    dashboard_mode = st.sidebar.radio(
        "Bereich auswählen:",
        ["📊 Übersicht", "🏭 Maschinenstatus", "📋 Produktionsaufträge",
         "📈 Analytics", "🚨 Alarme", "⚙️ Einstellungen"]
    )
    ...
    st.sidebar.selectbox(
        "Zeitraum:",
        ["Letzte Stunde", "Letzten 4 Stunden", "Letzten 8 Stunden",
         "Letzten 24 Stunden", "Aktuelle Schicht"]
    )
...
def show_overview(machine_df, orders_df, kpis):
    ...
    with col4:
        ...
        st.metric(
            "🚨 Aktive Alarme",
            f"{alarms}",
            help="Anzahl aktiver Warnungen und Fehler"
        )
    ...
    with status_col2:
        ...
        if not running_orders.empty:
            for _, order in running_orders.iterrows():
                progress = (order['quantity_completed'] / order['quantity_planned']) * 100
    ...
    hourly_production = machine_df.groupby([
        machine_df['timestamp'].dt.floor('H'), 'machine'
    ])['parts_produced'].sum().reset_index()
    ...
    with temp_col:
        ...
        fig_temp.add_hline(y=70, line_dash="dash", line_color="red",
    ...
    with power_col:
        ...
        power_data = machine_df.groupby('machine')['power_consumption'].last().reset_index()
        fig_power = px.pie(
            power_data,
...
def show_machine_status(machine_df):
    ...
    for _, machine in latest_status.iterrows():
        with st.container():
            ...
            status_color = "🟢" if machine['status'] == 'Normal' else "🟡" if machine['status'] == 'Warnung' else "🔴"
            ...
            with st.expander(f"📊 Details für {machine['machine']}"):
                ...
                with detail_col1:
                    ...
                    st.write(f"**Letztes Update:** {machine['timestamp'].strftime('%H:%M:%S')}")

                with detail_col2:
                    # Verlaufsdiagramm für diese Maschine
                    machine_history = filtered_df[filtered_df['machine'] == machine['machine']].tail(20)

                    fig_history = make_subplots(
                        rows=2, cols=2,
                        subplot_titles=('Temperatur', 'Leistung', 'Geschwindigkeit', 'Qualität'),
                        vertical_spacing=0.1
                    )

                    fig_history.add_trace(
                        go.Scatter(x=machine_history['timestamp'], y=machine_history['temperature'],
                                 name='Temperatur', line={"color": 'red'}),
                        row=1, col=1
                    )

                    fig_history.add_trace(
                        go.Scatter(x=machine_history['timestamp'], y=machine_history['power_consumption'],
                                 name='Leistung', line={"color": 'blue'}),
                        row=1, col=2
                    )

                    fig_history.add_trace(
                        go.Scatter(x=machine_history['timestamp'], y=machine_history['cutting_speed'],
                                 name='Geschwindigkeit', line={"color": 'green'}),
                        row=2, col=1
                    )

                    fig_history.add_trace(
                        go.Scatter(x=machine_history['timestamp'], y=machine_history['quality_index'],
                                 name='Qualität', line={"color": 'orange'}),
                        row=2, col=2
                    )
    ...
    comparison_metric = st.selectbox(
        "Vergleichsmetrik:",
        ["temperature", "power_consumption", "cutting_speed", "quality_index", "parts_produced"]
    )
    ...
    comparison_df = filtered_df.groupby(['timestamp', 'machine'])[comparison_metric].mean().reset_index()
...
def show_production_orders(orders_df):
    ...
    status_summary = filtered_orders.groupby('status').agg({
        'order_id': 'count',
        'quantity_planned': 'sum',
        'quantity_completed': 'sum'
    }).rename(columns={'order_id': 'Anzahl Aufträge'}).reset_index()
    ...
    filtered_orders['progress'] = (
        filtered_orders['quantity_completed'] / filtered_orders['quantity_planned'] * 100
    ).fillna(0)

    # Formatierte Anzeige
    display_orders = filtered_orders.copy()
    display_orders['start_time'] = display_orders['start_time'].dt.strftime('%d.%m.%Y %H:%M')
    display_orders['estimated_end'] = display_orders['estimated_end'].dt.strftime('%d.%m.%Y %H:%M')
    ...
    planning_orders = filtered_orders[filtered_orders['status'].isin(['Geplant', 'Läuft', 'Wartend'])]
    ...
    with st.expander("➕ Neuen Auftrag hinzufügen"):
        with st.form("new_order_form"):
            ...
            with col2:
                new_machine = st.selectbox("Maschine", orders_df['machine_assigned'].unique())
...
def show_analytics(machine_df, orders_df):
    ...
    hourly_kpis = filtered_analytics.groupby(
        filtered_analytics['timestamp'].dt.floor('H')
    ).agg({
        'parts_produced': 'sum',
        'quality_index': 'mean',
        'temperature': 'mean',
    ...
    correlation_data = filtered_analytics[
        ['temperature', 'power_consumption', 'cutting_speed',
         'parts_produced', 'quality_index']
    ].corr()
    ...
    machine_comparison = filtered_analytics.groupby('machine').agg({
    ...
    machine_comparison.columns = [
        'Gesamt Produktion', 'Ø Produktion/Zeitraum', 'Ø Qualität [%]',
        'Ø Temperatur [°C]', 'Ø Leistung [%]', 'Ø Geschwindigkeit [m/min]'
    ]
...
def show_alarms(machine_df):
        ...
        for _, alarm in current_alarms.head(10).iterrows():
            ...
            with st.container():
                ...
                with alarm_col1:
                    ...
                    st.write(f"**Zeit:** {alarm['timestamp'].strftime('%d.%m.%Y %H:%M:%S')}")
                ...
                with alarm_col3:
                    if st.button("✅ Quittieren", key=f"ack_{alarm['machine']}_{alarm['timestamp']}"):
    ...
    last_24h = machine_df[machine_df['timestamp'] >= datetime.now() - timedelta(hours=24)]
    alarm_stats = last_24h.groupby(['machine', 'status']).size().reset_index(name='count')
    ...
    hourly_alarms = machine_df.groupby([
        machine_df['timestamp'].dt.floor('H'), 'status'
    ]).size().reset_index(name='alarm_count')
    ...
    with st.expander("⚙️ Alarm-Einstellungen"):
        ...
        with config_col1:
            st.number_input("Temperatur Warnung [°C]", value=70, min_value=50, max_value=100)
            st.number_input("Temperatur Kritisch [°C]", value=85, min_value=70, max_value=120)

            st.number_input("Qualität Warnung [%]", value=95.0, min_value=90.0, max_value=99.0, step=0.1)

        with config_col2:
            st.number_input("Leistung Warnung (Min) [%]", value=30, min_value=10, max_value=50)
...
def show_settings():
    ...
    users_data = {
        ...
        'Letzter Login': ['2024-09-04 08:30', '2024-09-04 06:00', '2024-09-03 22:15', '2024-09-04 09:00'],
    ...
    with backup_col1:
        ...
        if backup_enabled:
            st.time_input("Backup-Zeit", value=datetime.now().time().replace(hour=2, minute=0))
    ...
    with backup_col2:
        ...
        st.multiselect(
            ...
            ["Maschinendaten", "Produktionsaufträge", "Qualitätsdaten", "Alarm-Historie"],
