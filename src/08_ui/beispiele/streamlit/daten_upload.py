...
st.set_page_config(
    page_title="Bystronic Daten-Upload",
    page_icon="📁",
    layout="wide"
)
...
def main():
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            tab1, tab2, tab3, tab4 = st.tabs(["📊 Übersicht", "🔍 Exploration", "🧹 Bereinigung", "📤 Export"])
    ...
    if numeric_cols:
        ...
        if selected_numeric:
            viz_type = st.selectbox(
                "Visualisierungstyp:",
                ["Histogramm", "Box Plot", "Scatter Plot", "Korrelationsmatrix", "Zeitreihe"]
            )
            ...
            if viz_type == "Histogramm":
                ...
                col2 = st.selectbox("Y-Achse:", [c for c in selected_numeric if c != col1])
                ...
                fig = px.imshow(corr_matrix, title="Korrelationsmatrix",
...
def show_data_cleaning(df):
    ...
    if len(missing_cols) > 0:
        ...
        for col in missing_cols.index:
            ...
            action = st.selectbox(
                f"Aktion für {col}:",
                ["Beibehalten", "Zeilen löschen", "Mittelwert", "Median", "Modus", "Konstante"],
                key=f"action_{col}"
            )
    ...
    if numeric_cols:
        ...
        if len(outliers) > 0:
            fig_box = px.box(cleaned_df, y=outlier_col, title=f"Ausreißer in {outlier_col}")
...
def show_export_options(df):
    ...
    export_df = df
    if data_source == "Bereinigte Daten (falls verfügbar)" and 'cleaned_data' in st.session_state:
    ...
    selected_cols = st.multiselect(
        "Spalten für Export (alle wenn leer):",
