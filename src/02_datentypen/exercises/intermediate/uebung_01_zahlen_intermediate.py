...
def validiere_eingabe(wert: str, typ: str, min_wert: Optional[float] = None, 
                     max_wert: Optional[float] = None) -> Union[int, float]:
    """
...
def berechne_produktionsstatistiken(teile_pro_tag: List[int]) -> dict:
    ...
    stats['variationskoeffizient'] = (stats['standardabweichung'] / stats['durchschnitt']) * 100
    
    return stats


def qualitaetskontrolle_komplex(messungen: List[float], sollwert: float, 
                               toleranz: float) -> Tuple[List[bool], dict]:
    """
    ...
    cp = (2 * toleranz) / (6 * standardabweichung) if standardabweichung > 0 else float('inf')
    
    # Cpk-Wert (Prozesslage)
    cpk_oben = (sollwert + toleranz - mittelwert) / (3 * standardabweichung) if standardabweichung > 0 else float('inf')
    cpk_unten = (mittelwert - (sollwert - toleranz)) / (3 * standardabweichung) if standardabweichung > 0 else float('inf')
...
def berechne_maschinenlaufzeit(start_zeit: float, end_zeit: float, 
                              pausen: List[Tuple[float, float]]) -> dict:
    """
    ...
    return {
        ...
        'auslastung_prozent': (effektive_laufzeit / gesamtzeit) * 100 if gesamtzeit > 0 else 0
    }
