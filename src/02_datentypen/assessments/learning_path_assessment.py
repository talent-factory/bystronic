...
class LearningPathAssessment:
    ...
    def starte_assessment(self) -> Dict:
        ...
        for kategorie, fragen in self.fragen.items():
            ...
            for i, frage_data in enumerate(fragen, 1):
                ...
                while True:
                    try:
                        antwort = input(f"\nIhre Antwort (1-{len(frage_data['optionen'])}): ").strip()
    ...
    def _zeige_ergebnis(self, gesamtpunkte: float, kategorie_punkte: Dict, lernpfad: Dict):
        ...
        for kategorie, punkte in kategorie_punkte.items():
            max_punkte = sum(max(f['optionen'], key=lambda x: x[1])[1] for f in self.fragen[kategorie])
            prozent = (punkte / max_punkte) * 100
            print(f"{kategorie.replace('_', ' ').title()}: {punkte}/{max_punkte} ({prozent:.0f}%)")
    ...
    def _speichere_ergebnis(self, gesamtpunkte: float, kategorie_punkte: Dict, lernpfad: Dict) -> Dict:
        ...
        dateiname = f'src/02_datentypen/assessments/results/assessment_result_{timestamp}.json'
