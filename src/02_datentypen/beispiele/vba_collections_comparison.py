...
----------------------""")
    ...
    # Auswertungen
    durchschnitts_gehalt = sum(m["gehalt"] for m in mitarbeiter.values()) / len(mitarbeiter)
...
""")
    ...
    wochendaten = {
        ...
        "tage": {
            "montag": {
                "datum": "2024-03-18",
                "schichten": {
                    "tag": {"stunden": 8, "mitarbeiter": 3, "maschinen": ["LASER_001", "PRESSE_001"]},
                    "nacht": {"stunden": 8, "mitarbeiter": 2, "maschinen": ["LASER_001"]}
                },
    ...
    zusammenfassung = {
        ...
        "ausschuss_rate": sum(gesamt_ausschuss) / (sum(laser_produktion) + sum(presse_produktion)) * 100
    }
    ...
    for tag, daten in wochendaten["tage"].items():
        for maschine in ["LASER_001", "PRESSE_001"]:
            ...
            effizienzen.append((tag, maschine, produktion, qualitaetsrate, verfügbarkeit))
    ...
    for tag, maschine, teile, qualitaet, verfuegbar in effizienzen:
        print(f"{tag:<10} {maschine:<12} {teile:<6} {qualitaet:>6.1f}% {verfuegbar:>8.1f}%")
    ...
    print(f"Schlechteste Verfügbarkeit: {schlechteste_verfügbar[1]} am {schlechteste_verfügbar[0]} ({schlechteste_verfügbar[4]:.1f}%)")
