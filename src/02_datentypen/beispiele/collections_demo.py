    ...
    produktionsanlage = {
        ...
        "maschinen": {
            "LASER_001": {
                "typ": "ByStar Fiber",
                "status": "Aktiv",
                "wartung": {"letzter_service": "2024-01-15", "naechster_service": "2024-07-15"},
                "produktion": {"heute": 145, "diese_woche": 720, "diesen_monat": 3200}
            },
            "PRESSE_002": {
                "typ": "Xpert Pro",
                "status": "Wartung", 
                "wartung": {"letzter_service": "2024-02-20", "naechster_service": "2024-08-20"},
                "produktion": {"heute": 0, "diese_woche": 450, "diesen_monat": 2100}
            }
        },
        "personal": [
            {"name": "Max Mustermann", "schicht": "Tag", "qualifikationen": ["Laser", "Presse"]},
            {"name": "Anna Müller", "schicht": "Nacht", "qualifikationen": ["Laser", "CNC"]}
        ]
    }
...
def erstelle_produktionsplan():
    ...
    maschinen_faehigkeiten = {
        "LASER_001": {"materialien": {"Stahl", "Aluminium", "Edelstahl"}, "max_dicke": 20},
    ...
    # Aufträge
    auftraege = [
        {"id": "A001", "teil": "Gehäuse", "material": "Stahl", "dicke": 5, "anzahl": 100},
        {"id": "A002", "teil": "Deckel", "material": "Aluminium", "dicke": 3, "anzahl": 150},
        {"id": "A003", "teil": "Bracket", "material": "Edelstahl", "dicke": 2, "anzahl": 200}
    ]
...
def zeige_produktionsplan(plan):
    ...
    for auftrag_id, daten in plan.items():
        ...
        print(f"   {auftrag_id}: {auftrag['teil']} ({auftrag['material']}, {auftrag['dicke']}mm)")
    ...
    for i in range(50):
        ...
        for eigenschaft, sollwert in sollwerte.items():
            if random.random() < 0.1:  # 10% Ausreisser
                abweichung = random.uniform(-toleranzen[eigenschaft] * 2, 
                                          toleranzen[eigenschaft] * 2)
            else:
                abweichung = random.uniform(-toleranzen[eigenschaft] * 0.5, 
                                          toleranzen[eigenschaft] * 0.5)
...
def analysiere_qualitaetsdaten(messungen):
    ...
    for eigenschaft in ["dicke", "breite", "laenge"]:
        ...
        print(f"     {eigenschaft}: Ø{mittelwert:.2f} (Min: {minimum:.2f}, Max: {maximum:.2f})")
...
def verwalte_lager():
    ...
    lagerbestand = {
        ...
        "Kupfer_2mm": {"bestand": 50, "einheit": "kg", "mindestbestand": 80}  # Unter Mindestbestand
    }
    ...
    kritische_materialien = {
        material for material, daten in lagerbestand.items() 
